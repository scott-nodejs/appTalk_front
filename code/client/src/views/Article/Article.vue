          ...
          <div class="article-view"
            ...
            <div class="article-title">
              ...
              <div class="author">
                <router-link :to="{
                  name: 'user',
                  params: { uid: article.user.id, routeType: 'article' }
                }"
                             class="avatar">
                  <img v-lazy="article.user.photo"
                       alt />
                </router-link>
                <div class="info">
                  <div class="name">
                  ...
                  <!-- 文章数据信息 -->
                  <div class="meta">
                    <span class="publish-time">{{article.dateString}}</span>
                    <span class="views-count">阅读 {{ article.readCount }}</span>
                    <span class="comments-count">评论 {{ article.commentCount }}</span>
                    <span class="likes-count">点赞 {{ article.applaudCount }}</span>
                    <span class="source">{{ sourceTypeList[1] }}
                      {{ articleTypeList[1] }}</span>
                  </div>
                </div>
              </div>
            </div>
    ...
    isThumb (item) {
      ...
      if (this.personalInfo.islogin) {
        if (
          ...
          ~this.user.associateInfo.articleThumdId.indexOf(item.id)
        ) {
          return true
        } else {
          return false
